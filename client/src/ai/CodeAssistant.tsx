  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
        role: "assistant", 
        content: data.response || "I can help you understand the codebase. What would you like to know?"
      }]);
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to query the repository. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
        ...
        {messages.length === 0 && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-md mx-auto">
              <button
                onClick={() => setQuery("What are the main components in this app?")}
              ...
              <button
                onClick={() => setQuery("How does the authentication work?")}
                className="p-3 bg-white/10 rounded-lg border border-white/20 hover:bg-white/20 transition-colors text-left"
              >
